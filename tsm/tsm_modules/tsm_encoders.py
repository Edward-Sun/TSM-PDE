...
@gin.configurable
def tsm_spectral_array_encoder(
    grid: grids.Grid,
    dt: float,
    physics_specs: physics_specifications.BasePhysicsSpecs,
    n_frames: int = 1,
  ...
  def encode_fn(inputs):
    bc = boundaries.periodic_boundary_conditions(grid.ndim)
    velocity = tuple(
        grids.GridVariable(grids.GridArray(x[-1], offset, grid), bc)
        for x, offset in zip(inputs, data_offsets))

    vorticity = cfd.finite_differences.curl_2d(velocity)
    vorticity0 = vorticity.data
